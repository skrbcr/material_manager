...
function link_url(cell, formatterParams) {
  ...
  return (
    '<a class="hlink_table" href="' + url + '" target="_blank">' + url + "</a>"
  );
}
...
document
  ...
  .addEventListener("click", async function () {
    if (
      bChanged &&
      !confirm("未保存の内容があります。放棄してファイルを開きますか？")
    ) {
      return;
    }
    const pickerOpts = {
